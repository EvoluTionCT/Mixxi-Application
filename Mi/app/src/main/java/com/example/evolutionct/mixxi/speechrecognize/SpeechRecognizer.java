package com.example.evolutionct.mixxi.speechrecognize;
...
import com.example.evolutionct.mixxi.javaFlacEncoder.FLACEncoder;
import com.example.evolutionct.mixxi.javaFlacEncoder.FLACStreamOutputStream;
import com.example.evolutionct.mixxi.javaFlacEncoder.StreamConfiguration;
...
public class SpeechRecognizer
{
	...
	final String API_KEY = "AIzaSyB7M5AJHjS45_mUmL0PyAElEHQ4gpskU-w "; // //AIzaSyA1-4UsPOu9Nbh1zka1bW1S_57yUd_yf8E //AIzaSyCwehYLphBwVupwglod95u4pLgg85WPLYo
	...
	public boolean checkRecording (){return recording;}
		...
		public void onPeriodicNotification(AudioRecord recorder)
		{
			...
			if (!recording) // Если запись не идёт...
			{
				if (voiceDetected) // И мы обнаружили вспышку амплитуды
				{
					...
					Log.d(TAG, "Voice record started" + "boo "+ recording);
			...
			if (recording) // Если запись идёт (или началась только что)
			{
				...
				if (!voiceDetected || (maxRecordLength < recordLength))
				{
					...
					if ((silenceLength >= maxSilenceLength) || (maxRecordLength < recordLength))
					{
						...
						recording = false;
						Log.d(TAG, "Voice record stopped, length: " + (recordLength - silenceLength) + " boo: "+ recording);

						/*Animation fadeOut = new AlphaAnimation(1, 0);
						//fadeOut.setInterpolator(new AccelerateInterpolator()); //and this
						fadeOut.setStartOffset(7000);
						fadeOut.setDuration(1000);
						AnimationSet animations = new AnimationSet(true);
						animations.addAnimation(fadeOut);
						CDA.ll.setAnimation(animations);

						CDA.ll2.setAnimation(animations);
						CDA.ll.setVisibility(LinearLayout.GONE);
						CDA.ll2.setVisibility(LinearLayout.GONE);*/
